...
class SubsequenceExample:
    fragment: str
    # read: Tuple[str, str, str] # TODO: Fix it before new training
    read: Tuple[str, str]
...
class FastaUniformSampler(IterableDataset):
    def __init__(
        ...
        sampling_strategy: Literal[
            "random_subsequence", "random_subsequence_uppercase", "three_nonoverlapping_subsequence", "three_nonoverlapping_subsequence_uppercase"
        ] = "random_subsequence",
    ...
    def iter_three_nonoverlapping_subsequence(self):
        while True:
            # Sample the fragment
            L_1 = torch.randint(self.range_min, self.range_max, (1,)).int()
            i_1 = torch.randint(low=0, high=int(self.len_text) - int(L_1), size=(1,))
            ...
            # sample the read from the start of the fragment
            L_2 = torch.randint(
                self.subsequence_range_min, self.subsequence_range_max, (1,)
            ).int()
            # sample the start of the second sequence from the first sequence [0, L_1//3 - L_2]
            i_2 = torch.randint(0, max(0,int(L_1//3)-L_2), (1,))
            read_1 = fragment[i_2 : i_2 + L_2]

            # sample the read from the middle of the fragment
            L_3 = torch.randint(
                self.subsequence_range_min, self.subsequence_range_max, (1,)
            ).int()
            i_3 = torch.randint(int(L_1//3), max(i_2 + L_2 + L_3,int(L_1//3)*2) - L_3, (1,))
            read_2 = fragment[i_3 : i_3 + L_3]
            
            
            # sample the read from the end of the fragment
            L_4 = torch.randint(
                self.subsequence_range_min, self.subsequence_range_max, (1,)
            ).int()
            i_4 = torch.randint(int(L_1//3)*2, max(i_3 + L_3 + L_4,L_1-L_4), (1,))
            read_3 = fragment[i_4 : i_4 + L_4]

            yield SubsequenceExample(fragment, (read_1, read_2, read_3), self.read_regularizer)
            
    def iter_three_nonoverlapping_subsequence_uppercase(self):
        for subsequence_example in self.iter_three_nonoverlapping_subsequence():
            fragment = subsequence_example.fragment
            read_1, read_2, read_3 = subsequence_example.read
            yield SubsequenceExample(fragment.upper(), (read_1.upper(), read_2.upper(), read_3.upper()), self.read_regularizer)
    ...
    def __iter__(self):
        ...
        elif self.sampling_strategy == "three_nonoverlapping_subsequence":
            return self.iter_three_nonoverlapping_subsequence()
        
        elif self.sampling_strategy == "three_nonoverlapping_subsequence_uppercase":
            return self.iter_three_nonoverlapping_subsequence_uppercase()
...
def collate_fn_sft(
    ...
    if read_regularization: #TODO: comment this out
        ...
        return SubsequenceExample(
            ...
            read=(read_1_tokenized, read_2_tokenized),
...
def add_noise_triplet(
    ...
    for read1, read2, read3 in batch:

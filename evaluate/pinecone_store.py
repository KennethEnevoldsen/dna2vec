    ...
    def trigger_pinecone_upsertion(
        ...
        for file_path in file_paths:
            ...
            for _, (batch, namespace) in tqdm(enumerate(batches)):
                ids = [
                    PineconeStore.generate_random_string() for _ in range(len(batch))
                ]
    ...
    def query_batch(
        ...
        def query_single(xq, query, index, single_hotstart):
            if not prioritize:
                ...
                xc = self.index.query(
                    xq, top_k=top_k, include_metadata=True, namespace=single_hotstart
                )
        ...
        if hotstart_list is None:
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(query_single, xq, query, index, None)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(query_single, xq, query, index, single_hotstart)
                    for xq, query, index, single_hotstart in zip(
                        xqs, queries, indices, hotstart_list
                    )
                ]
...
if __name__ == "__main__":
    ...
    pinecone_obj = PineconeStore(device="cuda:3", index_name=args.indexname)

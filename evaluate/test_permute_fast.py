...
import random
from aligners.smith_waterman import (
    bwamem_align_parallel,
    bwamem_align,
    bwamem_align_parallel_process,
)
...
def main(
    ...
    for i, line in enumerate(test_lines):
        ...
        sub_index, substring = identify_random_subsequence(query, 300)
        sub_indices.append(sub_index)
        queries.append(substring)
        indices.append(index)

    full_indices = [
        int(ind_base) + int(ind_fine)
        for ind_base, ind_fine in zip(indices, sub_indices)
    ]
    # finer_flag = main_align(
    #     store, queries, full_indices, top_k, flex=True, distributed=True
    # )

    results, incorrect_aligned = main_align(
        store,
        queries,
        full_indices,
        top_k,
    ...
    old_len = len(results)
    print("PERFORMANCE ON FIRST PASS: ", total_perf)

    # finer_flags[:, start : start + generalize] = finer_flag.reshape(-1, 1)
    # start += generalize

    # np.savez_compressed(
    #     f"test_cache/permute/run_{config}_{test_k}_{top_k}_{generalize}.npz",
    #     finer=finer_flags,
    # )
...
if __name__ == "__main__":
    ...
    for store, data_alias, config in initialize_pinecone(
        checkpoint_queue, data_queue, args.device
    ):

            ...
            for (
                ...
                if cfg.test_mode:
                    ...
                    if cfg.return_type == "score":
                        ...
                        # if not True:
                        queries_read_str_list = [query.read.query_sequence for query in mapped_reads]
                        bwa_mem2_align(fasta_file_path, queries_read_str_list, "/home/mehmet/codebase/dna2vec/evaluate/aligners", f"{log_folder}/test_bwa_mem2.sam")
                        # read the sam file and get the starting indices of alignments
                        starting_indices = []
                        with open(f"{log_folder}/test_bwa_mem2.sam", "r") as sam_file:
                            for line in sam_file:
                                if not line.startswith("@"):
                                    fields = line.strip().split("\t")
                                    if fields[1] == "0":
                                        starting_indices.append(fields[3])
                                    
                        #convert to int
                        starting_indices = np.array([int(index) for index in starting_indices])
                        starting_indices_gt = np.array([query.read.reference_start for query in mapped_reads])
                        
                        #compare elements of starting_indices and starting_indices_gt
                        accuracy = np.mean(starting_indices == starting_indices_gt)
                        print(f"BWA-MEM2 Accuracy: {accuracy:.4f}")
                        print("DNA2VEC Accuracy: ", total_perf)
                        
                        starting_indices -= 1
                        accuracy_modified = np.mean(starting_indices == starting_indices_gt)
                        print(f"BWA-MEM2 Accuracy (modified): {accuracy_modified:.4f}")
                        print("DNA2VEC Accuracy: ", total_perf)
                        
                        # save these metrics to a file
                        with open(f"{log_folder}/bwa_mem2_accuracy.txt", "w") as f:
                            f.write(f"BWA-MEM2 Accuracy: {accuracy:.4f}\n")
                            f.write(f"BWA-MEM2 Accuracy (modified): {accuracy_modified:.4f}\n")
                            f.write(f"DNA2VEC Accuracy: {total_perf:.4f}\n")

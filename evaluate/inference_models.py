...
class Baseline:
    def __init__(
        ...
        if option == "dna2bert-mean" or option == "dna2bert-max":
            ...
            option == "nucleotide-transformer-max"
            or option == "nucleotide-transformer-mean"
        ):
            self.tokenizer = AutoTokenizer.from_pretrained(
            ...
            from inference_model_helpers import (
                CharacterTokenizer,
            ...
            if self.option == "nucleotide-transformer-mean":
                ...
                return (
                    torch.nn.functional.normalize(
                        mean_sequence_embeddings.squeeze(), dim=0
                    )
                    .detach()
                    .cpu()
                    .numpy()
                ...
                return (
                    torch.nn.functional.normalize(
                        max_sequence_embeddings.values.squeeze(), dim=0
                    )
                    .detach()
                    .cpu()
                    .numpy()
                ...
                tokens_ids = self.tokenizer(x, return_tensors="pt", padding=True)[
                    "input_ids"
                ]
                attention_mask = tokens_ids != self.tokenizer.pad_token_id
                ...
                for i, s in enumerate(tok_seq):
                    if (
                        len(s) < 1252
                    ):  # VERY MUCH AN EDGE CASE - 1 in a million chance. Due to chromosome cuts.
                        attention_mask[i][len(s) : 1252] = 0
                        s.extend([4] * (1252 - len(s)))

                # place on device, convert to tensor
                tok_seq = torch.LongTensor(
                    tok_seq
                )  # .unsqueeze(0)  # unsqueeze for batch dim
    ...
    def get_sentence_embedding_dimension(self):
        ...
        if self.option == "dna2bert-max" or self.option == "dna2bert-mean":
            ...
            self.option == "nucleotide-transformer-max"
            or self.option == "nucleotide-transformer-mean"
        ):
            return 1280
